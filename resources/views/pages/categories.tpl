...
{block name="content"}
    ...
    <section class="container category-directory">
        {foreach $categories as $category}
            <a class="category-tile" href="{$category.url}"><span class="eyebrow">{$category.articles|count} статьи</span><h2>{$category.name}</h2><p>{$category.description}</p><span class="text-link">Смотреть статьи →</span></a>
        {/foreach}
    </section>
{/block}
